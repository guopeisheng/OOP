...
public class CollideStrategy implements IUpdateStrategy<IBallCmd> {
	//only collide ball run updateCollision method,then change direction and interactWith function(some balls' interactWith is empty though
	//hence, only collide ball will trigger collision, normal ball will not. They will collision when touch collide ball, but they are passive
	...
	public void updateState(final Ball context, IDispatcher<IBallCmd> dispatcher) {
		dispatcher.dispatch(new IBallCmd() { //give this command to all balls in dispatcher
			...
			public void apply(Ball other, IDispatcher<IBallCmd> disp) {
				if (context != other) {
					if (context.getLocation().distance(other.getLocation()) <= context.getRadius()
							+ other.getRadius()) {
	...
	protected void updateCollision(Ball context, Ball target, double x, double y, IDispatcher<IBallCmd> dispatcher) {
		...
		context.getVelocity().translate((int) Math.round(x / prevContext), (int) Math.round(y / prevContext));
