...
import paint.shape.EllipseShapeFactory;

/**
 *ellipse paint strategy
 *
 */
public class EllipsePaintStrategy extends ShapePaintStrategy {
	...
	public EllipsePaintStrategy() {
		// note that the at object is created in the lower layer and transfered to the upper layer (to ShpaePaintStrategy
		// and then to ApaintStrategy)
	...
	public EllipsePaintStrategy(AffineTransform at, double x, double y, double width, double height) {
		// extends the paintXfrm will transform the fixed ellipse to the given radius and loc from the ballworld
